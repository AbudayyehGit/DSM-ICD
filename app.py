...
# ==========================================
st.set_page_config(
    page_title="Taxonomy Utility Navigator",
    page_icon="🧬",
    layout="wide"
)
...
with st.sidebar:
    ...
    # Check if key exists in Streamlit Secrets, otherwise ask user
    ...
    api_key = st.text_input(
        ...
        value=default_key, 
        type="password", 
        help="OpenAI API key or key for your custom endpoint."
    )
    
    model_name = st.text_input(
        "Model Name", 
        value="gpt-4o", 
        help="e.g., gpt-4o, gpt-4o-mini, or a local model name."
    )
    
    base_url = st.text_input(
        "Custom Base URL (Optional)", 
        placeholder="https://api.openai.com/v1", 
        help="Leave blank for OpenAI, or enter a custom endpoint."
    )
...
if st.button("Run Analysis", type="primary"):
    if not api_key:
        st.error("Please enter an API Key in the sidebar or set it in Streamlit Secrets.")
    elif not subject_input:
        st.warning("Please enter a subject to analyze.")
    else:
        try:
            with st.spinner(f"Analyzing {subject_input} through the diagnostic taxonomy..."):
                ...
                response = client.chat.completions.create(
                    ...
                    messages=[
                        ...
                        {"role": "user", "content": f"Analyze this subject: {subject_input}"}
                    ]
                )
            ...
            st.error("The model failed to return structured JSON. Try running the request again.")
